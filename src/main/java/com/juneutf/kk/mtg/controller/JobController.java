...
import com.juneutf.kk.mtg.service.JobService;
...
public class JobController {
	...
	public String postEditID(JobModel jobModel, Model model) {
		try {
			...
			if (jobUpdate == 1) {
				...
				return "redirect:/kk/mtg/getjob?id=" + jobModel.getId();
			} else {
				log.warn("編集予約内容機能はエラーが発生します。");
				return "error";
			}
	...
	public String postDeleteID(JobModel jobModel, Model model) {
		...
		if (jobUpdate == 1) {
			...
			return "redirect:/kk/mtg/job?id=" + jobModel.getId();
		}
	}
